...
def _make_installer(
    slug: str = "test-pack", session=None, show_progress: bool = False, **source_kwargs
):
    source = CurseForgeSource(api_key="fake-key", slug=slug, **source_kwargs)
    modpack = ModpackConfig(platform="curseforge", source=source)
    return CurseForgePackInstaller(source, modpack, session=session, show_progress=show_progress)


def _add_slug_search_mock(mod_id: int = 555, slug: str = "test-pack") -> None:
    rsps_lib.add(
        rsps_lib.GET,
        f"{_API}/v1/mods/search",
        json={"data": [{"id": mod_id, "slug": slug}]},
    )
...
@rsps_lib.activate
def test_resolve_by_slug_specific_file_id(tmp_path):
    mod_id = 555
    file_id = 1001
    mod_file = _make_mod_file(file_id)
    pack_zip = _make_modpack_zip(_minimal_manifest())
    ...
    rsps_lib.add(
        rsps_lib.GET,
        f"{_API}/v1/mods/{mod_id}/files/{file_id}",
        json={"data": mod_file},
    )
    ...
    session = build_session()
    result = _make_installer(session=session, file_id=file_id).install(tmp_path)
    assert result["name"] == "Test Pack"
...
def test_install_no_slug_no_file_id_raises():
    from pydantic import ValidationError

    with pytest.raises(ValidationError, match="slug.*file_id"):
        CurseForgeSource(api_key="fake-key")
...
def test_install_global_slug_excludes_mod(tmp_path, tmp_path_factory, monkeypatch):
    ...
    monkeypatch.setattr(
        cf_mod, "load_exclude_include", lambda _kind: {"globalExcludes": ["jei"], "modpacks": {}}
    )
