...
- [x] Update `mc-helper setup` dispatch in `cli.py` to handle `server_pack` config
...
- [x] `mods/modrinth.py` — resolve slug/ID/version → download JAR
...
- [x] `mods/curseforge.py` — resolve slug/project-ID/file-ID → download JAR
...
- [x] Parallel download in `setup` command (ThreadPoolExecutor, max 10 workers)
