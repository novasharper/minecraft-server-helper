...
In `mods` mode, all Modrinth and CurseForge mod downloads are submitted to a `ThreadPoolExecutor` with a maximum of 10 concurrent workers. URL-based mods are downloaded sequentially after the pool finishes. Errors from individual mods are collected and reported at the end; a non-zero exit code is returned if any download failed.
